...
The following plot demonstrates the relationship between the natural log of sustainability (in the form of the sustainability development index) and natural log of gross domestic product per capita for high (greater than or equal to 20000 dollars) GDP per capita countries over the years 1990 to 2019. The form of the relationship appears linear with the points following a straight line. The direction is negative with an increase in GDP per capita in 2017 dollars resulting in a decrease in sustainability development index (SDI). The strength is moderately strong with a slight amount of scatter around the line. Overall, there does not appear to be any unusual observations in either plots. Over time, the sustainability index decreased and the GDP per Capita in 2017 dollars increased.
...
The statistical method that we selected was the simple linear regression. A simple linear regression assumes that the relationship between the gross domestic product per capita and the sustainability development index has a linear function for the form. This was deemed an appropriate method because the relationship between the two updated variables are linear, the observations do not depend on one another, the residuals were distributed normally, and there is constant variability.
...
The slope is -0.00067. We estimate that for every 10000 increase in a county's GDP per capita, their sustainability development index will decrease by 6.7 units.
...
4.  **Equal Variance of Residuals** - the residuals are roughly spaced out evenly with respected to the fitted values (values along the line).
...
The linear regression model explains about 39% (98.56726) of the total variance (251.129) in sustainability (SDI). This R-squared value is generally considered to be relatively low, indicating that the quality of the model is less than ideal. Additionally, the standard deviation of the residuals (sigma) is 12.5, indicating that there is a fair amount of variability in the residuals.
